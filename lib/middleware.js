...
exports.logger = function (logger, verbose = false) {
  return async function (context, next) {
    ...
    await next()
    logger.debug({ context, duration: Date.now() - requestTime }, 'outgoing response')
    if (verbose) {
...
exports.sessionLoader = () => {
  return async (ctx, next) => {
    ...
    // not really session relevant, but needed for csrf validation
    if (ctx.request.body && ctx.request.body.parsed) {
      ctx.payload = ctx.request.body.parsed

      if (ctx.request.body.parsed._csrf) {
        ctx.request.body._csrf = ctx.request.body.parsed._csrf
      }
    }
